...
public class Utils
{
	...
	static void blacklistTileEntity(Class<? extends TileEntity> tileEntity)
	{
		blacklistedBlockEntities.add(tileEntity);
	}
	...
	static void blacklistString(String string)
	{
		if (string.indexOf(':') == -1)
		{
			...
			{
				LOGGER.info("Class not found: " + string + ", ignoring");
			}
		}
		else
		{
	...
	{
		return new ResourceLocation("torcherino", path);
	}
}
