...
public class TorcherinoTileEntity extends TileEntity implements INameable, ITickableTileEntity
{
	...
	@Override public ITextComponent getName(){ return hasCustomName() ? customName : new TranslationTextComponent(world.getBlockState(pos).getBlock().getTranslationKey()); }
	...
	private void tickBlock(BlockPos blockPos)
	{
		...
		if (tileEntity == null || tileEntity.isRemoved() || TorcherinoAPI.INSTANCE.isTileEntityBlacklisted(tileEntity.getType()) || !(tileEntity instanceof ITickableTileEntity)) return;
