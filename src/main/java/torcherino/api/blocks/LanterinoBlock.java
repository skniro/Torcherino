...
public class LanterinoBlock extends CarvedPumpkinBlock implements BlockEntityProvider
{
    ...
    public LanterinoBlock(int maxSpeed, Identifier id)
    {
    ...
    @Override
    public void onPlaced(World world, BlockPos pos, BlockState oldState, LivingEntity placer, ItemStack handStack)
    {
        if (world.isClient) return;
        String prefix = "Something";
        if (placer != null) prefix = placer.getDisplayName().getString() + " (" + placer.getUuidAsString() + ")";
        Utils.LOGGER.info("[Torcherino] {} placed a {} at {} {} {}.", prefix, StringUtils
                .capitalize(getTranslationKey().replace("block.torcherino.", "").replace("_", " ")), pos.getX(), pos.getY(), pos.getZ());
    }
